...
export default function SociosPage() {
  ...
  async function loadPendingDebits(memberId: string) {
    ...
    try {
      ...
      debs.sort((a, b) => {
        const af = new Date(a.fecha).getTime(); const bf = new Date(b.fecha).getTime();
  ...
  async function handleSubmitCharge() {
    ...
    try {
      ...
      for (const it of payItems) {
        ...
        const srv = services.find(s => s.id === it.servicioId);
        const tipo = (srv?.tipo || it.tipoServicio || 'MENSUAL') as 'MENSUAL'|'ANUAL'|'UNICO';
  ...
  const isUnifiedFormValid = () => {
    ...
    if (!unifiedForm.fecha || !unifiedForm.servicioId || !unifiedForm.monto) {
      return false;
    }

    // Validar monto numérico
    const monto = parseFloat(getNumericValue(unifiedForm.monto));
    ...
    if (unifiedForm.condicion === 'CONTADO') {
      ...
      if (unifiedForm.permitirPagosParc) {
        if (!unifiedForm.pagoAplicado) {
          return false; // Si permite pago parcial, debe tener monto
        }
        
        const pagoAplicado = parseFloat(getNumericValue(unifiedForm.pagoAplicado));
        if (isNaN(pagoAplicado) || pagoAplicado <= 0) {
          return false; // Monto de pago parcial debe ser válido
        }
        
        if (pagoAplicado > monto) {
          return false; // Pago parcial no puede superar el monto total
        }
        
        if (!unifiedForm.metodoPago) {
          return false; // Pago parcial requiere método de pago
        }
      }
    }
  ...
  const handleUnifiedContadoPayment = async () => {
    ...
    setSaving(true);
    try {
      // 1. Crear el movimiento de débito
      const srv = services.find(s => s.id === unifiedForm.servicioId);
      const concepto = srv?.nombre || unifiedForm.concepto || 'Servicio';
      const monto = parseFloat(getNumericValue(unifiedForm.monto));
      const days = unifiedForm.tipoServicio === 'ANUAL' ? 365 : unifiedForm.tipoServicio === 'MENSUAL' ? 30 : (unifiedForm.dias || 1);
      
      // Crear suscripción si es necesario
      let subId = '';
      if (unifiedForm.crearSuscripcion && unifiedForm.tipoServicio !== 'UNICO') {
        const subRes = await AuthClient.authenticatedFetch(
          `/api/members/${encodeURIComponent(selectedMemberForService.id)}/subscriptions`,
          {
            method: 'POST',
            body: JSON.stringify({
              serviceId: unifiedForm.servicioId,
              periodicity: unifiedForm.tipoServicio,
              price: monto,
              autoDebit: false,
              notes: `Creado desde formulario unificado el ${unifiedForm.fecha}`,
              validUntil: unifiedForm.vencimiento || addDays(unifiedForm.fecha, days),
            }),
          }
        );
        const subJson = await subRes.json().catch(() => ({}));
      ...
      const debitRes = await AuthClient.authenticatedFetch('/api/movements', {
        ...
        body: JSON.stringify({
          ...
          concepto: `${concepto}${unifiedForm.tipoServicio !== 'UNICO' ? ` (${unifiedForm.tipoServicio})` : ''}`,
          tipo: 'DEBIT',
          monto: Number(monto),
          origen: 'SERVICIO',
          refId: unifiedForm.servicioId,
          observaciones: unifiedForm.observaciones || `Pago contado - ${unifiedForm.metodoPago}`,
          vencimiento: unifiedForm.tipoServicio === 'UNICO' ? unifiedForm.fecha : (unifiedForm.vencimiento || addDays(unifiedForm.fecha, days)),
        }),
      });
      const debitJson = await debitRes.json().catch(() => ({}));
      if (!debitRes.ok) throw new Error(debitJson?.msg || 'No se pudo crear el débito del servicio');
      ...
      const payRes = await AuthClient.authenticatedFetch('/api/payments', {
        ...
        body: JSON.stringify({
          ...
          fecha: unifiedForm.fecha,
          monto: monto,
          concepto: `Pago contado: ${concepto}`,
          ...
          allocations: [{ debitId: newDebitId, amount: monto }],
        }),
      });
      const payJson = await payRes.json().catch(() => ({}));
      ...
      if (unifiedForm.presupuestoDeudas && parseFloat(getNumericValue(unifiedForm.presupuestoDeudas)) > 0) {
        ...
        const debtPayRes = await AuthClient.authenticatedFetch('/api/payments', {
          ...
          body: JSON.stringify({
            ...
            fecha: unifiedForm.fecha,
            monto: presupuesto,
            concepto: `Pago de deudas pendientes - ${concepto}`,
            formaPago: unifiedForm.metodoPago,
            cobradorId: unifiedForm.cobradorId || '',
            numeroRecibo: unifiedForm.numeroRecibo || '',
  ...
  const handleUnifiedCreditoService = async () => {
    ...
    try {
      const srv = services.find(s => s.id === unifiedForm.servicioId);
      const concepto = srv?.nombre || unifiedForm.concepto || 'Servicio';
      const montoTotal = parseFloat(getNumericValue(unifiedForm.monto));
      const days = unifiedForm.tipoServicio === 'ANUAL' ? 365 : unifiedForm.tipoServicio === 'MENSUAL' ? 30 : (unifiedForm.dias || 1);
      ...
      const debitRes = await AuthClient.authenticatedFetch('/api/movements', {
        ...
        body: JSON.stringify({
          ...
          origen: 'SERVICIO',
          refId: unifiedForm.servicioId,
          observaciones: unifiedForm.observaciones || 'Servicio a crédito',
          vencimiento: unifiedForm.tipoServicio === 'UNICO' ? unifiedForm.fecha : (unifiedForm.vencimiento || addDays(unifiedForm.fecha, days)),
        }),
      });
      const debitJson = await debitRes.json().catch(() => ({}));
      ...
      // Si hay pago parcial, crear el pago
      if (unifiedForm.permitirPagosParc && unifiedForm.pagoAplicado && unifiedForm.metodoPago) {
        ...
        const payRes = await AuthClient.authenticatedFetch('/api/payments', {
          ...
          body: JSON.stringify({
            ...
            observaciones: unifiedForm.observaciones || '',
            allocations: [{ debitId: newDebitId, amount: pagoAplicado }],
          }),
        });
        const payJson = await payRes.json().catch(() => ({}));
  ...
  const handleDebitSelection = (debitId: string, isSelected: boolean) => {
    ...
    // Actualizar monto a pagar automáticamente
    const total = newSelectedDebits.reduce((sum, id) => {
      const debit = pendingDebits.find(d => d.id === id);
      if (debit) {
          ...
          >
            <div className="p-6">
              ...
              <div className="space-y-6">
                ...
                {/* ===== 1. DATOS DEL SERVICIO (Siempre visible) ===== */}
                <div className="bg-white border border-gray-200 rounded-xl p-4">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          const newVencimiento = updateVencimientoAutomatico(newFecha, unifiedForm.tipoServicio);
                          setUnifiedForm(prev => ({ 
                            ...prev, 
                            fecha: newFecha,
                    ...
                    <div className="lg:col-span-2">
                      <label className="block text-xs font-medium text-gray-700 mb-1">Servicio/Concepto *</label>
                      <select
                        value={unifiedForm.servicioId}
                        onChange={(e) => {
                          const selectedService = services.find(s => s.id === e.target.value);
                          const newTipoServicio = selectedService?.tipo || unifiedForm.tipoServicio;
                          const newVencimiento = updateVencimientoAutomatico(unifiedForm.fecha, newTipoServicio);
                          
                          setUnifiedForm(prev => ({
                            ...prev,
                            servicioId: e.target.value,
                            concepto: selectedService?.nombre || '',
                            monto: selectedService ? formatNumberWithSeparator(String(
                              selectedMemberForService?.subcategoria === 'NO SOCIO' 
                                ? selectedService.precioNoSocio || selectedService.precio 
                                : selectedService.precioSocio || selectedService.precio
                            )) : prev.monto,
                            tipoServicio: newTipoServicio as any,
                            vencimiento: newVencimiento
                          }));
                        }}
                        className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-gray-800 focus:border-gray-800"
                    ...
                    <div>
                      ...
                      <select
                        value={unifiedForm.tipoServicio}
                        onChange={(e) => {
                          const newTipoServicio = e.target.value as any;
                          const newVencimiento = updateVencimientoAutomatico(unifiedForm.fecha, newTipoServicio);
                          setUnifiedForm(prev => ({ 
                            ...prev, 
                            tipoServicio: newTipoServicio,
                            vencimiento: newVencimiento
                          }));
                        }}
                        className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-gray-800 focus:border-gray-800"
                    ...
                    <div>
                      ...
                      <input
                        type="text"
                        inputMode="numeric"
                        value={unifiedForm.monto}
                        onChange={(e) => {
                          const formatted = formatNumberWithSeparator(e.target.value);
                          setUnifiedForm(prev => ({ ...prev, monto: formatted }));
                        }}
                        className={`w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 ${
                          showFormValidation && (!unifiedForm.monto || parseFloat(getNumericValue(unifiedForm.monto)) <= 0)
                      ...
                      {showFormValidation && !unifiedForm.monto && (
                        <p className="text-xs text-red-600 mt-1">Monto requerido</p>
                      )}
                    </div>

                    {/* Días (solo para DIARIO) */}
                    {unifiedForm.tipoServicio === 'DIARIO' && (
                      <div>
                        <label className="block text-xs font-medium text-gray-700 mb-1">Días</label>
                        <input
                          type="number"
                          min="1"
                          value={unifiedForm.dias || 1}
                          onChange={(e) => {
                          const newDias = parseInt(e.target.value) || 1;
                          const newVencimiento = unifiedForm.tipoServicio === 'DIARIO' 
                            ? addDays(unifiedForm.fecha, newDias)
                            : unifiedForm.vencimiento;
                          setUnifiedForm(prev => ({ 
                            ...prev, 
                            dias: newDias,
                            vencimiento: newVencimiento
                    ...
                    {/* Vencimiento */}
                    <div className={unifiedForm.tipoServicio === 'DIARIO' ? '' : 'lg:col-span-2'}>
                      <label className="block text-xs font-medium text-gray-700 mb-1">Vencimiento</label>
                      <input
                        type="date"
                        value={unifiedForm.vencimiento}
                        onChange={(e) => setUnifiedForm(prev => ({ ...prev, vencimiento: e.target.value }))}
                        className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-gray-800 focus:border-gray-800"
                      />
                    </div>
                  ...
                  {unifiedForm.tipoServicio !== 'UNICO' && (
                    <div className="mt-4 p-3 bg-gray-50 rounded-lg">
                      <div className="flex items-center gap-4">
                        <label className="flex items-center gap-2">
                          <input
                            type="checkbox"
                            checked={unifiedForm.crearSuscripcion}
                            onChange={(e) => setUnifiedForm(prev => ({ ...prev, crearSuscripcion: e.target.checked }))}
                            className="w-4 h-4 text-blue-600"
                          />
                          <span className="text-sm">Crear suscripción</span>
                        </label>
                      </div>
                    </div>
                  )}
                    ...
                    <div>
                      ...
                      <div className="space-y-1 text-sm">
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium">Gs. {unifiedForm.monto ? parseInt(getNumericValue(unifiedForm.monto)).toLocaleString('es-PY') : '0'}</span>
                        </div>
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-6xl max-h-[90vh] overflow-hidden">
            ...
            <div className="bg-gradient-to-r from-amber-500 to-orange-500 px-6 py-4 text-white">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h2 className="text-xl font-bold">Gestión de Deudas Pendientes</h2>
            ...
            <div className="p-6 overflow-y-auto max-h-[calc(90vh-120px)]">
              ...
              <div className="bg-green-50 border border-green-200 rounded-lg p-6 mb-6">
                <h3 className="text-lg font-semibold text-green-800 mb-4">💳 Datos del Pago</h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  ...
                  {/* Monto a Pagar */}
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Monto a Pagar (Gs.) *</label>
                    <input
                      type="text"
                      inputMode="numeric"
                      value={debtPaymentForm.montoAPagar}
                      onChange={(e) => handleMontoAPagarChange(e.target.value)}
                      className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500"
                      placeholder="50.000"
                    />
                    <div className="text-xs text-gray-500 mt-1">
                      💡 Edita el monto para auto-asignar por antigüedad
                    </div>
                  </div>
                  ...
                  {/* Número de Recibo */}
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Nro. de Recibo/Factura</label>
                    <input
                      type="text"
                ...
                {/* Resumen del Pago - Solo si hay débitos seleccionados */}
                {selectedDebits.length > 0 && (
                  <div className="mt-4 bg-white border border-green-200 rounded-lg p-4">
                    <h4 className="text-sm font-semibold text-gray-900 mb-2">📊 Resumen del Pago</h4>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                      <div className="space-y-1">
                        <div className="flex justify-between">
                          <span className="text-gray-600">Débitos Seleccionados:</span>
                          <span className="font-medium">{selectedDebits.length}</span>
                        </div>
                        <div className="flex justify-between">
                      ...
                      <div className="space-y-1">
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium text-green-600">
                            Gs. {debtPaymentForm.montoAPagar ? parseInt(getNumericValue(debtPaymentForm.montoAPagar)).toLocaleString('es-PY') : '0'}
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-600">Saldo Restante:</span>
                          <span className={`font-medium ${
                            (calculateSelectedDebitsTotal() - (debtPaymentForm.montoAPagar ? parseInt(getNumericValue(debtPaymentForm.montoAPagar)) : 0)) <= 0
                              ? 'text-green-600' : 'text-orange-600'
                          }`}>
                            Gs. {Math.max(0, calculateSelectedDebitsTotal() - (debtPaymentForm.montoAPagar ? parseInt(getNumericValue(debtPaymentForm.montoAPagar)) : 0)).toLocaleString('es-PY')}
                          </span>
                        </div>
                      </div>
                      <div className="space-y-1">
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium">{debtPaymentForm.metodoPago || 'No seleccionado'}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-600">Estado:</span>
                          <span className={`font-medium ${
                            debtPaymentForm.montoAPagar && debtPaymentForm.metodoPago
                              ? 'text-green-600' : 'text-gray-500'
                          }`}>
                            {debtPaymentForm.montoAPagar && debtPaymentForm.metodoPago ? 'Listo para procesar' : 'Faltan datos'}
                          </span>
                        </div>
                      </div>
                    </div>
                    ...
                    {debtPaymentForm.montoAPagar && (
                      <div className="mt-3 p-3 rounded-lg text-sm">
                        {parseInt(getNumericValue(debtPaymentForm.montoAPagar)) > calculateSelectedDebitsTotal() ? (
                          <div className="bg-orange-100 border border-orange-300 text-orange-800">
                            ⚠️ <strong>Pago excedente:</strong> El monto supera la deuda seleccionada. El excedente se aplicará como crédito a favor.
                          </div>
                        ) : parseInt(getNumericValue(debtPaymentForm.montoAPagar)) < calculateSelectedDebitsTotal() ? (
                          <div className="bg-blue-100 border border-blue-300 text-blue-800">
                            💡 <strong>Pago parcial:</strong> Se pagará parcialmente los débitos seleccionados, priorizando los más antiguos.
                          </div>
                        ) : (
                          <div className="bg-green-100 border border-green-300 text-green-800">
                            ✅ <strong>Pago completo:</strong> Se saldarán todos los débitos seleccionados.
                          </div>
                        )}
                      </div>
                    )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                ...
                <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-sm text-gray-600">Débitos Pendientes</div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className="px-3 py-1.5 bg-amber-100 text-amber-800 rounded-md hover:bg-amber-200 transition-colors text-sm font-medium"
                      >
                        ✓ Seleccionar Todo
                      </button>
                      <button
                      ...
                      <button
                        ...
                        className="px-3 py-1.5 bg-blue-100 text-blue-800 rounded-md hover:bg-blue-200 transition-colors text-sm disabled:opacity-50"
                      >
                        {loadingDebits ? '⟳' : '🔄'} Actualizar
                      </button>
                    </div>
                  </div>

                  {selectedDebits.length > 0 && (
                    <div className="bg-white border border-green-200 rounded-md px-3 py-2">
              ...
              <div className="bg-white border border-gray-200 rounded-lg">
                <div className="px-4 py-3 border-b border-gray-200 bg-gray-50">
                  <h3 className="text-sm font-medium text-gray-900">📋 Detalle de Débitos Pendientes</h3>
                </div>

                <div className="divide-y divide-gray-200">
                  {loadingDebits ? (
                    <div className="p-8 text-center">
                      <div className="text-gray-500">⟳ Cargando débitos pendientes...</div>
                    </div>
                              ...
                              <div className="flex items-start justify-between">
                                <div className="flex-1">
                                  ...
                                  <div className="mt-1 flex flex-wrap items-center gap-2 text-sm text-gray-600">
                                    <span>📅 {debit.fecha ? new Date(debit.fecha).toLocaleDateString('es-PY') : 'N/A'}</span>
                                    {debit.vencimiento && (
                                      <span className={isVencido ? 'text-red-600 font-medium' : ''}>
                                        ⏰ Venc: {new Date(debit.vencimiento).toLocaleDateString('es-PY')}
                                        {isVencido && diasVencido > 0 && (
                                          <span className="ml-1 text-red-500">({diasVencido} días)</span>
                                        )}
                                      </span>
                                    )}
                                  </div>
                                ...
                                <div className="text-right ml-4">
                                  ...
                                  {(debit.paidAmount || 0) > 0 && (
                                    <div className="text-sm text-gray-500">
                                      Pagado: Gs. {(debit.paidAmount || 0).toLocaleString('es-PY')}
                                    </div>
                                  )}
                                  <div className="text-xs text-gray-400 mt-1">
                                    Total: Gs. {(debit.monto || 0).toLocaleString('es-PY')}
                                  </div>
                                </div>
                              </div>
                            </div>
                    ...
                    <div className="p-8 text-center">
                      <div className="text-gray-500 mb-2">📭 No se encontraron débitos pendientes</div>
                      <button
                        onClick={() => loadPendingDebits(selectedMemberForDebts.id)}
            ...
            <div className="bg-gray-50 border-t border-gray-200 px-6 py-4">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div className="flex items-center gap-4">
                  ...
                  {pendingDebits && pendingDebits.length > 0 && (
                    <button
                      onClick={() => {
                        if (debtPaymentForm.montoAPagar) {
                          const amount = parseInt(getNumericValue(debtPaymentForm.montoAPagar));
                          autoAssignByAge(amount);
                        }
                      }}
                      disabled={!debtPaymentForm.montoAPagar}
                      className="px-3 py-1 text-xs font-medium text-blue-700 bg-blue-100 border border-blue-200 rounded-md hover:bg-blue-200 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      🎯 Asignar por Antigüedad
                    </button>
                  )}
                </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  {selectedDebits.length > 0 && (
                    <>
                      <button
                        onClick={() => {
                          // Opción 1: Transferir la selección al modal principal de pagos/servicios
                          const totalSelected = calculateSelectedDebitsTotal();
                          setUnifiedForm(prev => ({
                            ...prev,
                            presupuestoDeudas: formatNumberWithSeparator(totalSelected.toString())
                          }));
                          
                          // Cerrar este modal y abrir el de cobranza
                          setShowDebtsModal(false);
                          setSelectedMemberForService(selectedMemberForDebts);
                          setShowChargeModal(true);
                          setActiveTab('unified');
                          setShowDebtAssignment(true); // Mostrar la sección expandida
                          setSelectedMemberForDebts(null);
                        }}
                        className="px-4 py-2 text-sm font-medium text-blue-700 bg-blue-100 border border-blue-200 rounded-lg hover:bg-blue-200 transition-colors"
                      >
                        📄 Ir a Pagos/Servicios
                      </button>

                      <button
                        onClick={() => {
                          // Opción 2: Procesar pago directamente desde este modal
                          console.log('Procesando pago de deudas:', {
                            member: selectedMemberForDebts,
                            selectedDebits,
                            paymentData: debtPaymentForm,
                            totalAmount: calculateSelectedDebitsTotal(),
                            paymentAmount: debtPaymentForm.montoAPagar ? parseInt(getNumericValue(debtPaymentForm.montoAPagar)) : 0
                          });
                          
                          // TODO: Implementar la llamada al API para procesar el pago
                          alert('Pago procesado correctamente!\n\n(Esta funcionalidad se implementará próximamente)');
                        }}
                        disabled={!debtPaymentForm.fecha || !debtPaymentForm.montoAPagar || !debtPaymentForm.metodoPago}
                        className="px-6 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed disabled:bg-gray-400 transition-colors font-medium"
                      >
                        💳 Procesar Pago
                        {debtPaymentForm.montoAPagar && (
                          <span className="ml-2 font-normal">
                            (Gs. {parseInt(getNumericValue(debtPaymentForm.montoAPagar)).toLocaleString('es-PY')})
                          </span>
                        )}
                      </button>
                    </>
