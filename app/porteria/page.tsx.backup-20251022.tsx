...
export default function Porteria() {
  ...
  // Estados para todas las reservas activas
  ...
  // Función para cargar todas las reservas activas
  const loadAllReservations = async (date?: string, status?: string) => {
    setLoadingAllReservations(true);
    try {
      const queryDate = date || selectedDate;
      const queryStatus = status || statusFilter;
      const response = await fetch(`/api/porteria/all-reservations?date=${queryDate}&status=${queryStatus}&limit=100`, {
        credentials: 'include'
      });
      ...
      if (result.ok) {
        ...
        setError(result.msg || 'Error al cargar las reservas');
      }
    } catch (error) {
  ...
  // Mostrar memberData en consola para diagnóstico
  useEffect(() => {
    if (memberData) {
              ...
              <div className="flex space-x-4">
                ...
                <button
                  onClick={() => {
                    ...
                    if (!allReservations) loadAllReservations();
                  }}
                  className={`px-4 py-2 rounded-lg font-medium ${
